...
public class IndividuoLegacyTests
{
    [Fact]
    ...
    public void Cruzar_ConOtraFamiliaCromosoma_LanzaInvalidOperationException()
    {
        ...
        var legacy = new IndividuoFake([1, 1, 2], problema, generador);
        var nuevo = new IndividuoOtraFamiliaFake([1, 1, 2], problema, generador);

        var ex = Assert.Throws<InvalidOperationException>(() => legacy.Cruzar(nuevo));
